...
mod tests {
    ...
    async fn full_cold_compaction_new_level_1_overlapping_with_level_2() {
        ...
        // pf4, L1, does not overlap with any but is small
        let builder = TestParquetFileBuilder::default()
            .with_line_protocol(&lp4)
            .with_max_seq(18)
            .with_min_time(26_000)
        ...
        // Cold compaction took files 1-5 and compacted them into file 7. File 6 was already L2 and
        // did not overlap with anything, so was untouched.
        assert_eq!(
            files_and_levels,
            vec![(6, CompactionLevel::Final), (7, CompactionLevel::Final),]
        );
