...
pub fn delete(i: &str) -> ParseResult<&str, DeleteStatement> {
...
mod test {
    ...
    fn test_delete() {
        ...
        let (_, got) = delete("DELETE FROM foo").unwrap();
        assert_eq!(format!("{}", got), "DELETE FROM foo");

        let (_, got) = delete("DELETE FROM foo WHERE time > 10").unwrap();
        assert_eq!(format!("{}", got), "DELETE FROM foo WHERE time > 10");

        let (_, got) = delete("DELETE WHERE time > 10").unwrap();
